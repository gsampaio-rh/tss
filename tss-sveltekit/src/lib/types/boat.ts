...
export class Boat {
  ...
  setTurn(turntype: TurnType): void {
    ...
    if (!this.forwardBtn || !this.tackBtn || !this.toMarkBtn) {
      console.log(`[BOAT.setTurn] Boat: ${this.name || 'Unnamed'}, Cannot set turn - buttons not initialized`);
      return;
    }
    
    console.log(`[BOAT.setTurn] Boat: ${this.name || 'Unnamed'}, Setting turntype to: ${TurnType[turntype]}`);
  ...
  saveTurn(turntype: TurnType, points: Point[], rotation: number, tack: boolean, finished: number | false): void {
    this.turns.push({
      turnType: turntype,
      points,
      rotation,
