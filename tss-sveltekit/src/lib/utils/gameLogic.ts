 ...
 */
export function getOptimalHeading(
	tack: boolean,
	windDir: number,
...
export function distanceToLine(
	...
	return Math.abs(Math.cos(rad) * (adjustedLY - adjustedY) - Math.sin(rad) * (lX - x));
}
 ...
 */
export function executeBoatTurn(
	boat: Boat,
	game: Game,
	...
	console.log(
		`  Initial State: tack=${initialTack}, rotation=${initialRotation.toFixed(1)}°, pos=(${initialX.toFixed(2)}, ${initialY.toFixed(2)}), turntype=${TurnType[initialTurnType]}`
	);
	...
	if (boat.turntype === TurnType.Tack) {
		...
		if (boat.tack) {
			...
			console.log(
				`  [TACKING] After: tack=PORT (true), rotation=${boat.rotation.toFixed(1)}° (45 + ${currentWind.toFixed(1)})`
			);
		} else {
			boat.rotation = -45 + currentWind;
			console.log(
				`  [TACKING] After: tack=STARBOARD (false), rotation=${boat.rotation.toFixed(1)}° (-45 + ${currentWind.toFixed(1)})`
			);
		}
		...
		if (!boat.finished) {
			...
			if (moveDist >= dist && game.isOutLaneline(boat.x, boat.y)) {
				...
				if (boat.y > game.marks[2].y) {
					const markDistance = distanceToLine(boat.x, boat.y, game.marks[2].x, game.marks[2].y, 0);
				...
				if (boat.x < 0.5) {
					console.log(
						`  [BOUNDARY] Boat hit left boundary (x=${boat.x.toFixed(2)}), forcing tack=PORT`
					);
					boat.tack = true;
				}
				if (boat.x > game.width - 0.5) {
					console.log(
						`  [BOUNDARY] Boat hit right boundary (x=${boat.x.toFixed(2)}), forcing tack=STARBOARD`
					);
					boat.tack = false;
				}
				...
				if (boat.turntype === TurnType.ToMark) {
					if (game.isOutLaneline(boat.x, boat.y)) {
						...
						if (oldTack !== boat.tack) {
							console.log(
								`  [TO MARK] Tack changed from ${oldTack} to ${boat.tack} (rotation=${boat.rotation.toFixed(1)}° > 0)`
							);
						}
					} else {
					...
					if (enableDirtyAirEffects) {
						...
						if (dirtyAirEffect.inDirtyAir) {
							...
							console.log(
								`  [DIRTY AIR] Intensity: ${(dirtyAirEffect.intensity * 100).toFixed(1)}%, Speed reduction: ${(speedReduction * 100).toFixed(1)}%, Angle adjustment: ${angleAdjustment.toFixed(1)}°`
							);
						}
					}
					...
					if (boat.tack) {
						console.log(
							`  [FORWARD] Port tack: rotation=${boat.rotation.toFixed(1)}° (base: ${(45 + currentWind).toFixed(1)}°)`
						);
					} else {
						console.log(
							`  [FORWARD] Starboard tack: rotation=${boat.rotation.toFixed(1)}° (base: ${(-45 + currentWind).toFixed(1)}°)`
						);
					}
			...
			console.log(
				`  [FINISHED] Boat already finished, moving with rotation=${boat.rotation.toFixed(1)}°`
			);
			boat.x += Math.sin((boat.rotation * Math.PI) / 180) * moveDist;
			boat.y -= Math.cos((boat.rotation * Math.PI) / 180) * moveDist;
			points.push({ x: boat.x, y: boat.y });
	...
	console.log(
		`  Final State: tack=${boat.tack}, rotation=${boat.rotation.toFixed(1)}°, pos=(${boat.x.toFixed(2)}, ${boat.y.toFixed(2)}), turntype=${TurnType[boat.turntype]}`
	);
	console.log(
		`  Movement: (${initialX.toFixed(2)}, ${initialY.toFixed(2)}) → (${boat.x.toFixed(2)}, ${boat.y.toFixed(2)})`
	);
	console.log(`  Points: ${points.length} points recorded`);

	boat.setTurn(boat.turntype);
	boat.saveTurn(boat.turntype, points, boat.rotation, boat.tack, boat.finished);
}
