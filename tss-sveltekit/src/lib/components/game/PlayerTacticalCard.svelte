<script lang="ts">
  ...
  $: liftHeaderResult = (windDir !== previousWindDirDeg && windwardMark && Math.abs(windDir - previousWindDirDeg) > 0.1) 
    ? calculateLiftHeader(boat, boat.x, boat.y, windwardMark.x, windwardMark.y, previousWindDirDeg, windDir, true)
    : null;
...
>
  ...
  <div class="card-header">
    <div class="player-identity">
      <span class="player-dot-large" style="background-color: {getBoatColorHex(boat.color)};"></span>
      <div class="player-info">
        <div class="player-name-row">
          <strong class="player-name">{boat.name || `Player ${playerIndex + 1}`}</strong>
          {#if position !== null}
            <span class="position-badge" class:first={position === 1} class:second={position === 2} class:third={position === 3}>
              #{position}
            </span>
          {/if}
        </div>
      ...
      {#if isLift || isHeader}
        ...
        {#if Math.abs(liftAmount) > 2}
          <div 
            ...
            class:lift={isLift}
            class:header={isHeader}
            style="{isLift ? 'right: ' + (50 - liftHeaderBarWidth / 2) + '%;' : 'left: ' + (50 - liftHeaderBarWidth / 2) + '%;'}"
          >
            {Math.abs(liftAmount).toFixed(0)}°
          </div>
        {/if}
      {:else}
        <div class="lift-header-bar-neutral">
          No shift
        </div>
      {/if}
    </div>
  ...
  <div class="metrics-grid">
    ...
    <div class="metric-card atw-metric">
      ...
      <div class="metric-value" class:green={atwColor === 'green'} class:yellow={atwColor === 'yellow'} class:red={atwColor === 'red'}>
        {formatCssDeg(atw)}
      </div>
    ...
    <div class="metric-card tack-metric" class:advantage={tackAdvantage < 0} class:disadvantage={tackAdvantage > 0}>
  ...
  <div class="secondary-info">
    ...
    <div class="info-item">
      ...
      <span class="info-value" class:underpowered={powerState === 'UNDERPOWERED'} class:overpowered={powerState === 'OVERPOWERED'}>
        {powerState}
      </span>
    </div>
...
<style>
  ...
  .lift-header-bar {
    ...
    width: 100%;
    height: 24px;
    background: linear-gradient(to right, rgba(220, 53, 69, 0.1) 0%, rgba(220, 53, 69, 0.1) 50%, rgba(40, 167, 69, 0.1) 50%, rgba(40, 167, 69, 0.1) 100%);
    border-radius: var(--border-radius-sm);
    border: 1px solid var(--color-border-light);
    overflow: visible;
  }
  ...
  .vmg-bar-fill-mini.good { background: #28a745; }
  .vmg-bar-fill-mini.ok { background: #ffc107; }
  .vmg-bar-fill-mini.poor { background: #dc3545; }
  ...
  .info-value.underpowered { color: #dc3545; }
  .info-value.overpowered { color: #ffc107; }
</style>
