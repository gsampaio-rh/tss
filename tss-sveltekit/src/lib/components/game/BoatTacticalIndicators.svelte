<script lang="ts">
  ...
  // 2. VMG
  $: vmg = windwardMark ? calculateVMG(boat.rotation, boat.x, boat.y, windwardMark.x, windwardMark.y, BOAT_SPEED) : 0;
  $: optimalVMG = BOAT_SPEED;
  $: vmgEfficiency = windwardMark ? calculateVMGEfficiency(boat.rotation, boat.x, boat.y, windwardMark.x, windwardMark.y, BOAT_SPEED, true) : 0;
  ...
  $: liftHeaderResult = (windDir !== previousWindDir && windwardMark) 
    ? calculateLiftHeader(boat, boat.x, boat.y, windwardMark.x, windwardMark.y, previousWindDir, windDir, true)
    : null;
  ...
  $: currentCourseAxis = windwardMark ? getCourseAxis(boat.x, boat.y, windwardMark.x, windwardMark.y) : 0;
  ...
  $: tackAdvantage = currentError < oppositeError ? (oppositeError - currentError) : -(currentError - oppositeError);
  $: tackAdvantagePercent = currentError + oppositeError > 0 ? Math.round((1 - currentError / (currentError + oppositeError)) * 100) : 0;
  ...
  $: decisionFlag = (() => {
    ...
    if (isHeader && Math.abs(liftAmount) > 8 && vmgEfficiency < 0.90) return 'TACK SOON';
    if (tackAdvantage < -5 && vmgEfficiency < 0.90) return 'TACK SOON';
    return 'HOLD';
  })();
...
<div class="boat-tactical-indicators">
  ...
  {#if decisionFlag !== 'HOLD'}
    <div class="indicator-clock indicator-12" class:urgent={decisionFlag === 'TACK NOW'} style="left: {pos12.x}px; top: {pos12.y}px;">
  ...
  <div class="indicator-clock indicator-1" style="left: {pos1.x}px; top: {pos1.y}px;">
    <div class="atw-indicator-compact">
      <div class="atw-value" class:green={atwColor === 'green'} class:yellow={atwColor === 'yellow'} class:red={atwColor === 'red'}>
        {formatCssDeg(atw)}
      </div>
  ...
  {#if isLift || isHeader}
    <div class="indicator-clock indicator-3" class:lift={isLift} class:header={isHeader} style="left: {pos3.x}px; top: {pos3.y}px;">
      <div class="lift-header-indicator-compact">
        ...
        <span class="lift-header-amount">{isLift ? '+' : '−'}{Math.abs(liftAmount).toFixed(0)}°</span>
      </div>
    </div>
  {/if}
  ...
  <div class="indicator-clock indicator-5" class:advantage={tackAdvantage < 0} class:disadvantage={tackAdvantage > 0} style="left: {pos5.x}px; top: {pos5.y}px;">
  ...
  <div class="indicator-clock indicator-6" style="left: {pos6.x}px; top: {pos6.y}px;">
    <div class="vmg-indicator-compact">
      ...
      <div class="vmg-percent-compact">
        {vmgPercent}%
        {#if vmgTrend !== 'stable'}
          <span class="vmg-trend-compact" class:up={vmgTrend === 'up'} class:down={vmgTrend === 'down'}>
            {vmgTrend === 'up' ? '▲' : '▼'}
          </span>
        {/if}
      </div>
  ...
  <div class="indicator-clock indicator-9" style="left: {pos9.x}px; top: {pos9.y}px;">
    <div class="heading-indicator-compact">
      ...
      <div class="heading-delta-compact">{headingDelta > 0 ? '+' : ''}{formatCssDeg(headingDelta)}</div>
    </div>
  </div>
...
<style>
  ...
  .atw-value.green { color: #28a745; }
  .atw-value.yellow { color: #ffc107; }
  .atw-value.red { color: #dc3545; }
  ...
  .vmg-bar-fill-compact.good { background: #28a745; }
  .vmg-bar-fill-compact.ok { background: #ffc107; }
  .vmg-bar-fill-compact.poor { background: #dc3545; }
  ...
  .vmg-trend-compact.up { color: #28a745; }
  .vmg-trend-compact.down { color: #dc3545; }
