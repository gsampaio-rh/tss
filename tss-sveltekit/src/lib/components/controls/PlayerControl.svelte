<script lang="ts">
  ...
  function handleTurnChange(turnType: TurnType) {
    console.log(`[PLAYER CONTROL] Boat: ${player.name || `Player ${playerIndex + 1}`}, Turn change: ${TurnType[turnType]}`);
    console.log(`  Current state: tack=${player.tack}, rotation=${player.rotation.toFixed(1)}°, turntype=${TurnType[player.turntype]}`);
...
<div class="player-control-group compact" class:finished={player.finished !== false}>
  ...
  {:else}
    ...
    <div class="input-group my-1 race-controls compact-controls" data-current-tack={player.tack ? 'port' : 'starboard'}>
      <span class="input-group-text">
        <div class="pn-control-color" style="background-color: {player.color};"></div>
      </span>
      ...
      <div class="btn-group" role="group">
        ...
        />
        <label 
          class="btn btn-outline-primary label-control {player.tack ? 'port-forward-btn' : 'starboard-forward-btn'}" 
          for="forward-{playerIndex}"
          title="Forward"
        >
          <svg xmlns="http://www.w3.org/2000/svg" class="icon-sm" viewBox="0 0 16 16" style="width: 16px; height: 16px;">
            {#if player.tack}
              <!-- Port Forward -->
              <polyline points="2,14 14,2" fill="none" stroke="currentColor" stroke-linejoin="miter" stroke-width="1.7" />
              <polyline points="13,8 14,2 8,3" fill="none" stroke="currentColor" stroke-linejoin="miter" stroke-linecap="round" stroke-width="1.7" />
            ...
            {:else}
              ...
              <polyline points="3,8 2,2 8,3" fill="none" stroke="currentColor" stroke-linejoin="miter" stroke-linecap="round" stroke-width="1.7" />
        ...
        />
        <label 
          class="btn btn-outline-primary label-control {player.tack ? 'port-tack-btn' : 'starboard-tack-btn'}" 
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" class="icon-sm" viewBox="0 0 16 16" style="width: 16px; height: 16px;">
            {#if player.tack}
              ...
              <polyline points="8,7 7,1 13,2" fill="none" stroke="currentColor" stroke-linejoin="miter" stroke-linecap="round" stroke-width="1.7" />
            ...
            {:else}
              ...
              <polyline points="8,7 9,1 3,2" fill="none" stroke="currentColor" stroke-linejoin="miter" stroke-linecap="round" stroke-width="1.7" />
              <ellipse rx="1.8" ry="1.8" cx="8" cy="14" fill="currentColor" />
            {/if}
          </svg>
        </label>
        ...
        <label 
          ...
          title="To Mark"
        >
          <svg xmlns="http://www.w3.org/2000/svg" class="icon-lg" viewBox="0 0 16 16" style="width: 20px; height: 20px;">
