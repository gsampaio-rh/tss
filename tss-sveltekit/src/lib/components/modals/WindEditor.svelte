			...
			<div class="modal-content">
				...
				<div class="modal-body row overflow-hidden">
					<div class="col" style="height: 100%; overflow: auto;">
						...
						<div class="mb-3 row">
							<div
								class="col d-block invalid-feedback"
								style="display: {errorText ? 'block' : 'none'} !important;"
							>
								{errorText}
							</div>
							<div class="col-auto"><span>Average wind: </span><span>{windAverage}</span></div>
							<div class="col-auto text-end">
								<span>Wind length: </span><span>{windCount}</span>
							</div>
						</div>
				...
				<div class="modal-footer">
					<div class="row w-100 g-2">
						<div class="col-auto">
							{#if !isCreate && !isReadOnly}
								<button type="button" class="btn btn-danger col-auto" on:click={handleDelete}>
									Delete
								</button>
							{/if}
						</div>
						<div class="col-auto">
							{#if !isCreate && !isReadOnly}
								<button class="btn btn-primary" on:click={handleCopy}> Make a copy </button>
							{/if}
						</div>
						<div class="col"></div>
